...
// src/completions/dashForce.ts
var import_coc2 = require("coc.nvim");
async function register(context, documentSelector) {
  if (import_coc2.workspace.getConfiguration("tailwindCSS").get("dashForceCompletions")) {
...
var DashForceCompletionProvider = class {
  async provideCompletionItems(_document, _position, _token, context) {
    if (context.triggerCharacter !== "-")
      return [];
    const dummyItems = [];
    import_coc2.workspace.nvim.call("coc#start");
    return dummyItems;
  }
};
...
// src/headwind/headwindFeature.ts
var config = import_coc3.workspace.getConfiguration();
...
function activate(context, outputChannel) {
  ...
  const disposable = import_coc3.commands.registerCommand("tailwindCSS.headwind.sortTailwindClasses", async () => {
...
`);
        ...
        const range = import_coc3.Range.create(
  ...
  const runOnProject = import_coc3.commands.registerCommand("tailwindCSS.headwind.sortTailwindClassesOnWorkspace", () => {
    const workspaceFolder = import_coc3.workspace.workspaceFolders || [];
    if (workspaceFolder[0]) {
      const workspacePath = import_coc3.Uri.parse(workspaceFolder[0].uri).fsPath;
      import_coc3.window.showInformationMessage(`Running Headwind on: ${workspacePath}`);
      const rustyWindArgs = [workspacePath, "--write", shouldRemoveDuplicates ? "" : "--allow-duplicates"].filter(
        (arg) => arg !== ""
      );
      const rustyWindBinPath = import_path.default.join(context.extensionPath, "node_modules", ".bin", "rustywind");
      const rustyWindProc = (0, import_child_process.spawn)(rustyWindBinPath, rustyWindArgs);
      ...
      rustyWindProc.stderr.on("data", (data) => {
        if (data && data.toString() !== "") {
          ...
          import_coc3.window.showErrorMessage(`Headwind error: ${data.toString()}`);
        }
      });
    }
  });
  ...
  if (config.get("tailwindCSS.headwind.runOnSave")) {
    context.subscriptions.push(
      import_coc3.workspace.onWillSaveTextDocument((_e) => {
        import_coc3.commands.executeCommand("tailwindCSS.headwind.sortTailwindClasses");
      })
    );
  }
}
...
// src/index.ts
var CONFIG_FILE_GLOB = "{tailwind,tailwind.config}.{js,cjs}";
var clients = /* @__PURE__ */ new Map();
var languages3 = /* @__PURE__ */ new Map();
var _sortedWorkspaceFolders;
function sortedWorkspaceFolders() {
  if (_sortedWorkspaceFolders === void 0) {
    _sortedWorkspaceFolders = import_coc4.workspace.workspaceFolders ? import_coc4.workspace.workspaceFolders.map((folder) => {
...
function getOuterMostWorkspaceFolder(folder) {
  ...
  for (const element of sorted) {
    ...
    if (uri.startsWith(element)) {
      const workdir = import_coc4.workspace.getWorkspaceFolder(element);
      if (workdir) {
        return workdir;
      }
    }
...
function getUserLanguages(folder) {
  const langs = folder ? import_coc4.workspace.getConfiguration("tailwindCSS", folder.uri.toString()).includeLanguages : void 0;
  return isObject(langs) ? langs : {};
}
function isExcluded(file, folder) {
  const exclude = getConfigExcludePatterns();
  for (const pattern of exclude) {
    if ((0, import_minimatch.default)(file, import_path2.default.join(import_coc4.Uri.parse(folder.uri).fsPath, pattern))) {
      return true;
    }
  }
  return false;
...
async function activate2(context) {
  if (!getConfigTailwindCssEnable())
    return;
  context.subscriptions.push(
    import_coc4.commands.registerCommand("tailwindCSS.forceActivate", async () => {
      import_coc4.window.showInformationMessage(`Force activation of coc-tailwindcss3`);
    })
  );
  ...
  if (module2 && import_fs.default.existsSync(module2)) {
    ...
    module2 = context.asAbsolutePath(
      import_path2.default.join("node_modules", "@tailwindcss", "language-server", "bin", "tailwindcss-language-server")
    );
  }
  const outputChannel = import_coc4.window.createOutputChannel("tailwindcss-language-server");
  context.subscriptions.push(
    import_coc4.commands.registerCommand("tailwindCSS.showOutput", () => {
      if (outputChannel) {
        outputChannel.show();
      }
    })
  );
  const watcher = import_coc4.workspace.createFileSystemWatcher(`**/${CONFIG_FILE_GLOB}`, false, true, true);
  watcher.onDidCreate((uri) => {
    let folder = import_coc4.workspace.getWorkspaceFolder(uri.toString());
    if (!folder) {
      return;
    }
  ...
  context.subscriptions.push(
    import_coc4.workspace.onDidChangeConfiguration((event) => {
      clients.forEach((client, key) => {
        const folder = import_coc4.workspace.getWorkspaceFolder(import_coc4.Uri.parse(key).toString());
        if (!folder)
          return;
        if (event.affectsConfiguration("tailwindCSS", folder.uri.toString())) {
          ...
          if (userLanguages) {
            ...
            if (!equal(newLanguages, languages3.get(folder.uri.toString()))) {
              languages3.set(folder.uri.toString(), newLanguages);
              if (client) {
  ...
  function bootWorkspaceClient(folder) {
    ...
    if (!languages3.has(folder.uri.toString())) {
      languages3.set(folder.uri.toString(), dedupe([...languages, ...Object.keys(getUserLanguages(folder))]));
    }
    const configuration = {
      edidor: import_coc4.workspace.getConfiguration("editor"),
      tailwindCSS: import_coc4.workspace.getConfiguration("tailwindCSS")
    };
    ...
    const serverOptions = {
      run: {
        module: module2,
        transport: import_coc4.TransportKind.ipc,
    ...
    const languageObj = languages3.get(folder.uri.toString());
    if (!languageObj)
      return;
    const documentSelector = languageObj.map((language) => ({
      scheme: "file",
      language,
      pattern: `${import_coc4.Uri.parse(folder.uri).fsPath}/**/*`
    }));
    const clientOptions = {
      documentSelector,
      ...
      outputChannel,
      synchronize: {
        fileEvents: import_coc4.workspace.createFileSystemWatcher(CONFIG_FILE_GLOB)
      },
      middleware: {
        workspace: {
          configuration: (params) => {
            return params.items.map(({ section, scopeUri }) => {
              let scope = folder;
              if (scopeUri) {
                const doc = import_coc4.workspace.textDocuments.find((doc2) => doc2.uri.toString() === scopeUri);
                if (doc) {
                  scope = {
              ...
              return import_coc4.workspace.getConfiguration(section);
            });
          }
        }
      },
    ...
    const client = new import_coc4.LanguageClient("tailwindCSS", "Tailwind CSS Language Server", serverOptions, clientOptions);
    client.start();
    clients.set(folder.uri.toString(), client);
    register(context, documentSelector);
  }
  async function didOpenTextDocument(document) {
    const uri = import_coc4.Uri.parse(document.uri);
    if (uri.scheme !== "file")
      return;
    const supportedLanguages = [];
    supportedLanguages.push(...languages);
    const includeLanguages = import_coc4.workspace.getConfiguration("tailwindCSS").get("includeLanguages");
    if (includeLanguages)
      supportedLanguages.push(...Object.keys(includeLanguages));
    if (!supportedLanguages.includes(document.languageId))
      return;
    let folder = import_coc4.workspace.getWorkspaceFolder(document.uri);
    if (!folder) {
      return;
    }
    folder = getOuterMostWorkspaceFolder(folder);
    try {
      const configFiles = await (0, import_fast_glob.default)(import_path2.default.join(import_coc4.Uri.parse(folder.uri).fsPath, "**/" + CONFIG_FILE_GLOB), {
  ...
  context.subscriptions.push(import_coc4.workspace.onDidOpenTextDocument(didOpenTextDocument));
  import_coc4.workspace.textDocuments.forEach(didOpenTextDocument);
  context.subscriptions.push(
    import_coc4.workspace.onDidChangeWorkspaceFolders((event) => {
